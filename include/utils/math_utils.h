...
inline Eigen::Matrix3d RotationFrom2Vec(Eigen::Vector3d from_vector,
                                        Eigen::Vector3d to_vector) {
  Eigen::Vector3d rotation_axis = from_vector.cross(to_vector);
  double rotation_angle = std::acos(from_vector.dot(to_vector) /
                                    from_vector.norm() / to_vector.norm());
  rotation_axis = (rotation_axis / rotation_axis.norm()).eval();
  Eigen::AngleAxisd rotation(rotation_angle, rotation_axis);
  return rotation.toRotationMatrix();
}

inline double CaculateAngle(Eigen::Vector3f v1, Eigen::Vector3f v2) {
  return acos(DotProduct(v1, v2) / (Normalize(v1) * Normalize(v2))) / M_PI *
         180.0;
}
